Shader "SEffect/UnLit Uber Base"
{
	Properties
    {
        ...
        Group#Main("基本参数", Int) = 1
            ...
            //-----------------------
/*
        Group#STRAIL("STrail[STRAIL]",Int) = 1
            _CustomSection("T/自定义截面[STRAIL]",Int) = 0
            _TrailWidth("宽度[STRAIL,!_CustomSection]",Float) = 1
            _TrailDir("E/朝向:面向相机,面向上方[STRAIL,!_CustomSection]",Int) = 0

        Group#_E_ADD_TEX("叠加贴图[_E_ADD_TEX]", Int) = 1
            _AddTex("叠加贴图[_E_ADD_TEX]", 2D) = "white" {}
            _AddCoordsys("E/坐标系:拉伸,稳定[_E_ADD_TEX,STRAIL]",Int) = 0           
            _DistortionByMainColor("UV被主贴图亮度影响[_E_ADD_TEX]",Float) = 0
            _AddRollUV("V/UV滚动:U向滚动,V向滚动[_E_ADD_TEX]", Vector) = (0, 0, 0, 0)
        ...
        Group#_E_MASK("遮罩[_E_MASK]", Int) = 1 
            ...
            _Mask_Distortion("T/影响扰动[_E_MASK,_E_DISTORTION]",Int) = 0           
            _Mask_Dissolve("T/影响溶解[_E_MASK,_E_DISSOLVE]",Int) = 0               
            _MaskTex("遮罩贴图[_E_MASK]", 2D) = "white" {}
            _MaskCoordsys("E/坐标系:拉伸,稳定[_E_MASK,STRAIL]",Int) = 0              
            _MaskRollUV("V/UV滚动:U向滚动,V向滚动[_E_MASK]", Vector) = (0, 0, 0, 0)

        Group#_E_DISTORTION("扰动[_E_DISTORTION]", Int) = 1
            _Distortion_AddTex("T/扰动叠加贴图[_E_DISTORTION,_E_ADD_TEX]",Int) = 1  
            _Distortion_MainTex("T/扰动主贴图[_E_DISTORTION,_E_ADD_TEX]",Int) = 1   
            _DistTex("扰动贴图 R:U向 G:V向(sRGB off)[_E_DISTORTION]", 2D) = "bump" {}
            _DistCoordsys("E/坐标系:拉伸,稳定[_E_DISTORTION,STRAIL]",Int) = 0       
            _DistRollUV("V/UV滚动:U向滚动,V向滚动[_E_DISTORTION]", Vector) = (0, 0, 0, 0)
            _Dist("V/扰动强度:U向强度(-1,1),V向强度(-1,1)[_E_DISTORTION]", Vector) = (0, 0, 0, 0)

        Group#_E_DISSOLVE("溶解[_E_DISSOLVE]", Int) = 1
            _DissolveChannel("E/通道:R,G,B,A[_E_DISSOLVE]", Int) = 0
            _DissTex("溶解贴图[_E_DISSOLVE]", 2D) = "white" {}
            _DissCoordsys("E/坐标系:拉伸,稳定[_E_DISSOLVE,STRAIL]",Int) = 0          
            _DissRollUV("V/UV滚动:U向滚动,V向滚动[_E_DISSOLVE]", Vector) = (0, 0, 0, 0)
            _Space(" /[_E_DISSOLVE]", Int) = 1
            _DissInt("溶解值[_E_DISSOLVE]", Range(0, 1)) = 0
            _DissFea("羽化过渡[_E_DISSOLVE]", Range(0.0001, 1)) = 0

        Group#_E_RIM("边缘光[_E_RIM]", Int) = 1
            [HDR]_RimColor("颜色[_E_RIM]", Color) = (1, 1, 1, 1)
            _RimOffset("偏移[_E_RIM]",Range(0,1)) = 0
            _RimSmooth("羽化[_E_RIM]",Range(0,1)) = 0.2

        Group#_E_FLIPBOOK("UV序列图[_E_FLIPBOOK]", Int) = 1
            _Tile("纹理排版 x:横向列数 y:纵向行数[_E_FLIPBOOK]", Vector) = (1, 1, 1, 1)
            _FlipbookIndex("当前序号[_E_FLIPBOOK]", Int) = 0    

        Group#_E_BILLBOARD("广告牌[_E_BILLBOARD]",Int) = 1
            _BillBoard_Y("T/只操作Y轴[_E_BILLBOARD]",Int) = 0

        Group#_E_FULLSCREEN("全屏显示[_E_FULLSCREEN]", Int) = 1
            _UseFixedRatio("T/固定比例[_E_FULLSCREEN]",Int) = 0
            _FixedRatio("x:横向偏移 y:竖向偏移 z:宽高比 w:缩放[_E_FULLSCREEN,_UseFixedRatio]",Vector) = (0,0,1,1)
*/
