Shader "SEffect/UnLit Uber Advance Base"
{
    Properties
    {
...
/*
        ...
        Group#_E_DISTORTION("扰动[_E_DISTORTION]", Int) = 1
            ...
            _DistCoordsys("E/坐标系:模型UV,极坐标,屏幕坐标,matcap[_E_DISTORTION]",Int) = 0
            _DistRollUV("V/UV滚动:U向滚动,V向滚动,固定旋转,匀速旋转[_E_DISTORTION]", Vector) = (0, 0, 0, 0)
            _Dist("V/扰动值:U向强度(-1,1),V向强度(-1,1),方向限制(-1,1),方向限制强度(0,1)[_E_DISTORTION]", Vector) = (0, 0, 0, 0)
        
        
        Group#_E_DISSOLVE("溶解[_E_DISSOLVE]", Int) = 1
            _Dissolve_AddTex("T/只溶解叠加贴图[_E_DISSOLVE,_E_ADD_TEX]",Int) = 1
            _DissolveChannel("E/通道:R,G,B,A[_E_DISSOLVE]", Int) = 0
            _DissTex("溶解贴图[_E_DISSOLVE]", 2D) = "white" {}
            _DissCoordsys("E/坐标系:模型UV,极坐标,屏幕坐标,matcap[_E_DISSOLVE]",Int) = 0
            _DissRollUV("V/UV滚动:U向滚动,V向滚动,固定旋转,匀速旋转[_E_DISSOLVE,!_DissCoordsys_3]", Vector) = (0, 0, 0, 0)
            _Space(" /[_E_DISSOLVE]", Int) = 1
            _DissInt("溶解值[_E_DISSOLVE]", Range(0, 1)) = 0
            _DissFea("羽化过渡[_E_DISSOLVE]", Range(0.0001, 1)) = 0.01
            _Space(" /[_E_DISSOLVE]", Int) = 1
            _Dissolve_Edge("T/溶解描边[_E_DISSOLVE]", Int) = 0
            [HDR]_DissColor1("亮边颜色[_E_DISSOLVE,_Dissolve_Edge]",Color) = (1,1,1,1)
            _DissColor2("暗边颜色[_E_DISSOLVE,_Dissolve_Edge]",Color) = (0,0,0,1)
            _DissOffset("描边偏移[_E_DISSOLVE,_Dissolve_Edge]", Range(0, 0.4)) = 0.1
            _DissSmooth("描边羽化[_E_DISSOLVE,_Dissolve_Edge]", Range(0.0001, 1)) = 0.01
